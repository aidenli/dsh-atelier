...
var Backend = class {
  ...
  async post(path, body, trusted = false) {
    this.checkOperation(
      path,
  ...
  checkOperation(path, allowed) {
    if (!allowed.some((name2) => path.split("?")[0] === `/${name2}`) || path.includes("..") || path.includes("\\"))
...
var installing;
async function updatePlugin(version) {
  if (installing) return installing;
  installing = (async () => {
    const info = await versionInfo();
    if (info.source) throw new Error("\u6E90\u7801\u52A0\u8F7D\u8BF7\u5148\u66F4\u65B0\u6E90\u7801\u5E76\u7F16\u8BD1\u90E8\u7F72");
    if (!info.hasUpdate || info.latest !== version || !/^\d+\.\d+\.\d+$/.test(version))
      throw new Error("\u66F4\u65B0\u7248\u672C\u5DF2\u53D8\u5316\uFF0C\u8BF7\u91CD\u65B0\u68C0\u67E5\u66F4\u65B0");
    const require2 = createRequire(resolve3(process.argv[1]));
    let cli;
    try {
      cli = resolve3(
        dirname2(require2.resolve("@deepseek-ai/dsh/package.json")),
        "lib/bin.js"
      );
    } catch {
    ...
    try {
      await promisify2(execFile2)(
        ...
        { windowsHide: true, timeout: 3e5, maxBuffer: 4 * 1024 * 1024 }
      );
    } catch {
...
async function versionInfo() {
  ...
  pending = (async () => {
    const root = resolve3(dirname2(fileURLToPath2(import.meta.url)), "..");
    const manifest = JSON.parse(
      await readFile3(resolve3(root, "package.json"), "utf8")
    );
    const info = {
      current: manifest.version,
      source: existsSync2(resolve3(root, "../backend/package.json")),
      hasUpdate: false
    };
    try {
      const response = await fetch(
        "https://registry.npmjs.org/dsh-atelier/latest",
        { signal: AbortSignal.timeout(5e3), redirect: "error" }
      );
      if (!response.ok) throw new Error();
      const metadata = await response.json();
      if (metadata.name !== "dsh-atelier" || typeof metadata.version !== "string" || !/^\d+\.\d+\.\d+$/.test(metadata.version))
        throw new Error();
...
// plugin/src/remote.ts
var _getVersion_dec, _a, _init;
var AtelierRemote = class extends (_a = TypertRemoteService, _getVersion_dec = [Remote], _a) {
  constructor(ctx) {
    super(ctx, "atelier");
...
__decorateElement(_init, 1, "getVersion", _getVersion_dec, AtelierRemote);
...
function registerMediaTools(ctx, backend, ownerId) {
  const disposers = [];
  const operations = {
    capabilities: "\u67E5\u8BE2 RunningHub \u5BC6\u94A5\u662F\u5426\u5DF2\u914D\u7F6E\uFF08hasApiKey\uFF09\u3001\u52A8\u4F5C\u8FC1\u79FB\u5DE5\u4F5C\u6D41\u548C\u53C2\u6570\u3002\u672A\u914D\u7F6E\u65F6\u5148\u63D0\u793A\u5230 Atelier \u670D\u52A1\u8BBE\u7F6E\u914D\u7F6E\uFF0C\u4E0D\u521B\u5EFA\u6216\u91CD\u8BD5\u4EFB\u52A1\u3002\u4EC5\u652F\u6301 Wan Animate2 \u5355\u6BB5\uFF0C\u56FA\u5B9A 30fps\u3002",
    ...
    disposers.push(
      ctx.tools.register(
        defineTool({
          ...
          description: operation === "task_create" ? "\u6BCF\u7EC4\u8F93\u5165\u521B\u5EFA\u4E00\u4E2A\u72EC\u7ACB\u52A8\u4F5C\u8FC1\u79FB\u9879\u76EE\u53CA\u4E00\u4E2A RunningHub \u4EFB\u52A1\uFF0C\u6279\u91CF\u4E09\u7EC4\u751F\u6210\u4E09\u4E2A\u9879\u76EE\uFF1B\u6279\u6B21\u53EA\u8D1F\u8D23\u5E42\u7B49\u4E0E\u6C47\u603B\u3002" + description : operation === "task_get" ? description + " \u8FD4\u56DE projectId \u6307\u5411\u6240\u5C5E\u4F5C\u54C1\uFF1B\u91CD\u8BD5\u4E0D\u521B\u5EFA\u65B0\u9879\u76EE\u3002" : description,
          parameters: {
            payload: {
              type: "string",
              required: true,
          ...
          async execute(args, execution) {
            ...
            switch (operation) {
              case "capabilities": {
                const config = await backend.get("/getConfig");
                result = {
                  hasApiKey: config.hasApiKey === true,
                  workflows: await backend.get("/listWorkflows"),
                  fps: 30,
                  autoStartWhenReady: true,
                  instructions: "\u672A\u914D\u7F6E API Key \u65F6\u5148\u63D0\u793A\u7528\u6237\u5230 Atelier \u670D\u52A1\u8BBE\u7F6E\u914D\u7F6E\uFF0C\u4E0D\u5728\u804A\u5929\u4E2D\u7D22\u53D6\u5BC6\u94A5\uFF1B\u914D\u7F6E\u540E\u91CD\u65B0\u68C0\u67E5\uFF0C\u518D\u67E5\u8BE2\u7D20\u6750\u5E76\u521B\u5EFA\u4EFB\u52A1\u3002\u5165\u961F\u540E\u65E0\u9700\u5FAA\u73AF\u67E5\u8BE2\u3002"
                };
                break;
              }
              case "asset_list":
...
function installTransferSkill(ctx, backend) {
  ...
  ctx.on("system-prompt/assemble", async (_assembly, context, next) => {
    ...
    let changed = false;
    if (pending2.delete(agent) && !context.signal?.aborted) {
...
async function apply(ctx, config) {
          ...
          async fetch(request) {
            try {
              ...
              if (operation === "getVersion") result = await versionInfo();
                ...
                result = method === "GET" ? await backend.get(path) : await backend.post(path, await request.json());
  ...
  for (const upload of [false, true])
    ctx.effect(
      () => ctx.connection.fetch.register({
        path: upload ? "/api/atelier.uploadAsset" : "/api/atelier.getAssetFile",
        ...
        async fetch(request) {
          ...
          const path = request.method === "POST" ? "/uploadAsset" : `/getAssetFile?id=${id}${url.searchParams.get("download") === "1" ? "&download=1" : ""}`;
          try {
            const init = {
